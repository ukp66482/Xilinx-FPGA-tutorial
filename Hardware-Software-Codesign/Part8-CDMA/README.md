...
不同於一般的 DMA (Part 7) 主要是處理 Stream 介面 (如 AXI-Stream to Memory)，CDMA 專注於 Memory-to-Memory 的傳輸。這裡的 Memory 是指系統中所有映射在實體位址上的儲存空間 (**AXI addressable memory**)，包含：

1. 主記憶體 (Memory, DDR)：容量最大，通常作為主記憶體。
...
| 介面名稱 | 方向 | AXI 類型 | 功能描述 |
|---------|------|----------|---------|
| S_AXI_LITE | Slave | AXI4-Lite | Control Interface,連接至 ZYNQ PS 的 GP Port。 CPU 透過這個介面讀寫 CDMA 的暫存器 (Registers)，例如設定來源地址、目的地址、搬運長度等。 |
| M_AXI | Master | AXI4 | Data Interface,連接至 ZYNQ PS 的 HP Port (High Performance)。CDMA 透過這個介面直接向 DDR 記憶體發出讀取與寫入請求，不需經過 CPU。 |
| M_AXI_SG | Master | AXI4 | Scatter Gather, CDMA 透過這個介面從記憶體中讀取 Descriptors。它讓 CDMA 能自動執行一連串不連續的搬運任務，而不需要 CPU 介入每一次傳輸。 | 
| cdma_introut | Output | not AXI | Interrupt, 當搬運完成或發生錯誤時，可發送 interrupt 通知 CPU 。 |
...
- Write/Read Data Width:
  ...
  - 建議配合 Zynq HP Port 的設定 (通常設為 64 或 32)，寬度越寬，單次 Cycle 搬運的資料量越大，頻寬越高。

- Write/Read Burst Size:

  - 決定一次 AXI Burst 的長度。數值越大 (如 16, 32, 64)，AXI Bus 利用率通常越高。
...
* `Scatter Gather`模式允許`CDMA`讀取位於記憶體中的`Descriptor Chain`，此次 lab 用不到
...
* 此 Address 是 `CPU` 用來檢查搬運結果的地方。當 `CDMA` 說搬運完成後，`CPU` 會讀取這個地址來驗證資料是否正確
* `Range`代表 `IP` 在系統 Memory Mapping 中所佔用的「地址空間大小」
* `BRAM Controller (axi_bram_ctrl_0)` 同時連接著 `Zynq CPU (processing_system7_0)` 以及 `CDMA (axi_cdma_0)`，因此兩邊的Range大小皆需要修改

11. 在 Jupyter 中建一個資料夾上傳 `example.ipynb`, `.bit` 檔, `.hwh` 檔, `example1.txt` (範例文檔)，並執行 `example.ipynb`。
